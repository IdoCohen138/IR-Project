...
#                 self._f.close()
    ...
    def upload_to_gcp(self):
        ...
        blob = self.bucket.blob(f"big_index_text/{file_name}")
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = self.bucket.get_blob(f"big_index_text_w_no_stemm{os.sep}{f_name}").open("rb")
...
TUPLE_SIZE = 14       # We're going to pack the doc_id and tf values in this 
                     # many bytes.
    ...
    def write_a_posting_list(b_w_pl, bucket_name):
        ...
        with closing(MultiFileWriter(".", bucket_id, bucket_name)) as writer:
            for w, pl in list_w_pl: 
                # convert to bytes
#                 b = c[(doc_id << 16 | (tf & TF_MASK)).to_bytes(TUPLE_SIZE, 'big')
#                               for doc_id, tf in pl])
                b = b''.join([struct.pack("IHfI", doc_id ,tf ,tf_idf_normalize ,doc_len) for doc_id ,tf ,tf_idf_normalize ,doc_len in pl])
                # write to file(s)
                locs = writer.write(b)
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name):
        ...
        blob_posting_locs = bucket.blob(f"big_index_text/{bucket_id}_posting_locs.pickle")
